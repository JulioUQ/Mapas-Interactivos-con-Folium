...
def gdf_gsa() -> gpd.GeoDataFrame:
    ...
    ruta_gsa = Path(r'P:\Proyectos\CALCULO_CIERRES\CONTROL CALIDAD\TeoríaSIPE\Programacion\Directorio GIS con QGIS\GIS_2.0\ZonasGSA\ZonasGSA.shp')
    ruta_gsa = Path(str(ruta_gsa))  # fuerza codificación compatible
    gdf = gpd.read_file(ruta_gsa)
    return gdf.to_crs(epsg=4326) if gdf.crs else gdf.set_crs(epsg=4326)

def gdf_FAO() -> gpd.GeoDataFrame:
    """Carga capa de FAO como GeoDataFrame en EPSG:4326."""
    ruta_fao = Path(r'P:\Proyectos\CALCULO_CIERRES\CONTROL CALIDAD\TeoríaSIPE\Programacion\Directorio GIS con QGIS\GIS_2.0\ZonasFAO\ZonasFAO.shp')
    ruta_fao = Path(str(ruta_fao))  # fuerza codificación compatible
...
def gdf_ZEE() -> gpd.GeoDataFrame:
    ...
    ruta_zee = Path(str(ruta_zee))  # fuerza codificación compatible
    gdf = gpd.read_file(ruta_zee)
    return gdf.to_crs(epsg=4326) if gdf.crs else gdf.set_crs(epsg=4326)

def gdf_LBR() -> gpd.GeoDataFrame:
    """Carga capa de LBR como GeoDataFrame en EPSG:4326."""
    ruta_lbr = Path(r'P:\Proyectos\CALCULO_CIERRES\CONTROL CALIDAD\TeoríaSIPE\Programacion\Directorio GIS con QGIS\GIS_2.0\Líneas de base\LBR.shp')
    ruta_lbr = Path(str(ruta_lbr))  # fuerza codificación compatible
    gdf = gpd.read_file(ruta_lbr)
    return gdf.to_crs(epsg=4326) if gdf.crs else gdf.set_crs(epsg=4326)
